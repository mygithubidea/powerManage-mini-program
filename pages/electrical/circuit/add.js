import {
    updateAppLineInspectionHistory,
    getAppLineInspectionHistory,
    uploadFile,
    downloadFile
} from '../../../api/api.js';
...
Page({
    ...
    gotoAddYhyd(e) {
        ...
        const safetyDefectContent = formData.lineName + formData.poleNumber + e.currentTarget.dataset.reason + "异常"
        const { managementUnit, inspector } = formData;
        this.setData({
            openKey: e.currentTarget.dataset.key
        })
        wx.navigateTo({
          url: `./yhyd?safetyDefectContent=${safetyDefectContent}&unit=${managementUnit}&inspector=${inspector}`,
        })
    },
    ...
    onLoad(options) {
        ...
        if (options?.id) {
            getAppLineInspectionHistory(options.id, res => {
                console.warn(res)
                const data = res.data;
