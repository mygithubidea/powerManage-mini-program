...
Page({
    data: {
        ...
        TabCur: 0,
        showMenu: [
            wx.getStorageSync('menu').includes("circuit1"),
            wx.getStorageSync('menu').includes("circuit2"),
            wx.getStorageSync('menu').includes("circuit3"),
        ],
        dataList0: [],
        dataList1: [],
        dataList2: [],
    ...
    query(index, isRefresh) {
        ...
        let queryFun = getAppLineInspectionTablePage;
        if (tdata.TabCur == 1) {
    ...
    goRecord(e) {
        ...
        wx.navigateTo({
            url: `./record?lineName=${dataset.linename}&poleNumber=${dataset.polenumber}`
        })
    },
    viewDefectRisk(e) {
        const dataset = e.currentTarget.dataset;
