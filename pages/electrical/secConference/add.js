...
Page({
    ...
    onLoad(options) {
        ...
        if (options?.id) {
            getAppSecurityMeetings(options.id, res => {
                console.warn(res)
                const data = res.data;
                data.minuteList = data.minute && data.minute.split(',').map(url => ({
                    url,
                    name: this.getFileName(url)
                }));
                data.meetingPhotosList = data.meetingPhotos && data.meetingPhotos.split(',').map(url => ({
                    url,
                    name: this.getFileName(url)
                }));
                ...
                if (data.meetingTime) {
                    data.date0 = data.meetingTime.split(' ')[0];
                    data.date1 = data.meetingTime.split(' ')[1];
                }
                this.setData({
                    formData: res.data
                })
    ...
    saveData() {
        ...
        formData.minute = formData.minuteList.map(z => z.url).join(',');
        formData.meetingPhotos = formData.meetingPhotosList.map(z => z.url).join(',');
        formData.signInSheet = formData.signInSheetList.map(z => z.url).join(',');
        const meetingTime = formData.date0 + " " + (formData.date1 || '');
        wx.showToast({
            title: "保存中",
            icon: "loading"
        })
